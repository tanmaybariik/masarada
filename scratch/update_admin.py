...
slider_state = """  // Slider State
  ...
  // Product Modal State"""
content = content.replace('  // Product Modal State', slider_state)
...
content = content.replace('        {[\n          { id: "analytics", label: "Analytics", icon: TrendingUp },', nav_pills)
...
content = content.replace('      {/* ========================================================= */}\n      {/* TAB 2: REVENUE, SALES & ACCOUNTING */}', slider_tab)
...
content = content.replace('      {/* ========================================================= */}\n      {/* MODAL: ADD / EDIT LIBRARY ARTICLE (TEXT & IMAGE) */}', slider_modal)


# 7. Replace inputs with ImageUploader
content = re.sub(
    r'<div>\s*<label className="font-bold text-foreground/80 block mb-1">Cover Image Path / Asset</label>\s*<input[^>]+value=\{articleForm.image\}[^>]+onChange=\{e => setArticleForm\(\{ \.\.\.articleForm, image: e\.target\.value \}\)\}[^>]+/>\s*</div>',
    '<ImageUploader label="Cover Image Path / Asset" value={articleForm.image} onChange={(val) => setArticleForm({ ...articleForm, image: val })} />',
    content
)

content = re.sub(
    r'<div>\s*<label className="font-bold text-foreground/80 block mb-1">Image URL / File Path</label>\s*<input[^>]+value=\{galleryForm.url\}[^>]+onChange=\{e => setGalleryForm\(\{ \.\.\.galleryForm, url: e\.target\.value \}\)\}[^>]+/>\s*</div>',
    '<ImageUploader label="Image URL / File Path" value={galleryForm.url} onChange={(val) => setGalleryForm({ ...galleryForm, url: val })} />',
    content
)

content = re.sub(
    r'<div>\s*<label className="font-bold text-foreground/80 block mb-1">Image Asset Path / URL</label>\s*<input[^>]+value=\{productForm.image\}[^>]+onChange=\{e => setProductForm\(\{ \.\.\.productForm, image: e\.target\.value \}\)\}[^>]+/>\s*</div>',
...
event_image = """              <ImageUploader label="Event Image" value={eventForm.image} onChange={(val) => setEventForm({ ...eventForm, image: val })} />

              <div>
                <label className="font-bold text-foreground/80 block mb-1">Venue / Location</label>"""
content = content.replace(event_venue, event_image)
